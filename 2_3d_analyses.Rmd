...
```{r}
#create slope for srtm (post_mining)
slope_srtm <- terrain(mines$Post_mining,opt='slope',unit='degrees')
...
```{r}
#create slope for ned (pre_mining)
slope_ned <-terrain(mines$Pre_mining, opt = 'slope', unit = 'degrees')

#add a dem difference layer
diff_ned_srtm <- slope_ned - slope_srtm
#plot(diff_ned_srtm)
slopes <- brick(slope_ned, slope_srtm, diff_ned_srtm)
#Rename raster layers
names(slopes) <- c("Pre_Mining", "Post_Mining", "Difference")
plot(slopes)
```
...
plot(mine_before_same)
plot(mine_after_same)

pre_mine_matrix <- matrix(raster::extract(mine_before_same,
                    raster::extent(mine_before_same),buffer=500),
                    nrow=ncol(mine_before_same), ncol=nrow(mine_before_same))
pre_mine_matrix %>%
  sphere_shade(sunangle = 360, texture = 'desert') %>%
  add_water(detect_water(pre_mine_matrix),color='desert') %>%
  add_shadow(ray_shade(pre_mine_matrix,zscale=4),0.8) %>%
  add_shadow(ambient_shade(pre_mine_matrix)) %>%
  plot_3d(pre_mine_matrix,zscale=10,fov = 0,theta=135,zoom=0.75)
...
```{r}
post_mine_matrix <- matrix(raster::extract(mine_after_same,
                    raster::extent(mine_after_same),buffer=500),
                    nrow=ncol(mine_after_same), ncol=nrow(mine_after_same))
post_mine_matrix %>%
  sphere_shade(texture = 'desert') %>%
  add_water(detect_water(post_mine_matrix),color='desert') %>%
  add_shadow(ray_shade(post_mine_matrix,zscale=2),0.5) %>%
  add_shadow(ambient_shade(post_mine_matrix)) %>%
  plot_3d(post_mine_matrix,zscale=10,fov = 0,theta=135,zoom=0.75)
