---
title: "Anthropogenic Topo Change"
...
#check units of az_mine
st_crs(az_mine)$units

az_mine_5km_buff <- st_buffer(az_mine, dist = 5000)
...
st_crs(az_mine_5km_buff)

az_mine_4326_5kmbuff <- az_mine_5km_buff %>%
  st_transform(4326)
...
mask_az_mine_buff <- mask(dem_diff, az_mine_4326_5kmbuff, inverse=TRUE) %>%
  plot(mask_az_mine_buff)
...
az_mine_epsg4326 <- az_mine %>%
  st_transform(4326)


mask_mine_exact <- mask(dem_diff, az_mine_epsg4326) %>%
  plot(mask_mine_exact)
...
unmined_dem <- getValues(mask_az_mine_buff)


mined_dem <- getValues(mask_mine_exact)
